	...
	<div class="container">
		<div class="col-md-12">
			...
			<div class="thumbnail" v-if="!loading">
				<img :src="item.thumbnail_url" alt="" class="img-responsive">
				...
				<div class="ratings">
					...
					<p class="pull-right">
						<button class="btn btn-success" @click="addItem" :disabled="item.quantity === 0">Add to cart</button>
					</p>
		...
		computed: {
			item() {
				...
				if (this.$store.getters.products.length >= id) {
					const filterArr = this.$store.getters.products.filter((item) => {
						return item.id == id;
					});
