<template>
	<div class="container" :class="{ loadingItem: loading }">
		...
		<div class="row action-panel" v-if="!loading">
			<div class="col-xs-12">
				<div class="btn-group pull-right">
					<a href="" id="list" class="btn btn-default btn-sm" @click.prevent="changeDisplay(true)">
						<span class="glyphicon glyphicon-th-list"></span>List
					</a>
					<a href="" id="grid" class="btn btn-default btn-sm" @click.prevent="changeDisplay(false)">
		...
		<div class="is-flex" v-if="!loading">
			<app-product-item v-for="(prod, idx) in products" :key="prod.id" :item="prod" :displayList="displayList"></app-product-item>
		</div>
	</div>
</template>
